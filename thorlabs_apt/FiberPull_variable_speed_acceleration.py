...
PullSpeed = float(input("Enter the desired initial pull speed: ")) #easily changeable pull speed
PullAcc = float(input("Enter the desired pull speed acceleration: ")) #easily changeable pull speed acceleration
...
motorLeft.backlash_distance = .01
motorRight.backlash_distance = .01

motorLeft.set_velocity_parameters(PullSpeed,PullAcc,2.6)
motorRight.set_velocity_parameters(PullSpeed,PullAcc,2.6)
motorLeft.move_by(pullLength/2)
motorRight.move_by(pullLength/2)
while (motorLeft.position < ((pullLength/2)+startPosition)*.9):
    pass
while (motorRight.position < (pullLength/2+startPosition)*.9):
    pass
...
while True:
    ...
    if command is 't':
        ...
        while motorRight.is_in_motion: 
            pass  
        leftPosition += tensionLength
        rightPosition += tensionLength
    elif command is 'u':
        ...
        while motorRight.is_in_motion: 
            pass
        leftPosition -= moveLength
        rightPosition += moveLength
    elif command is 'l':
