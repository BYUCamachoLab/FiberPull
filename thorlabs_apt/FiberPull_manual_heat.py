...
motorTorch.set_velocity_parameters(1.0,1.0,1.0) # min, acceleration, max ---mm/s/s
...
#---leave torch on fiber until user hits enter, then start pull
tic = time.time()
input("Press enter to start pullng")
...
moveIncrement = .1 #distance the stages move
...
for x in np.arange(start_pos, end_pos, moveIncrement):
    ...
    wait_completion_pull = true;
    while wait_completion_pull is True:
         if motorRight.is_in_motion is False:
             wait_completion_pull = False
