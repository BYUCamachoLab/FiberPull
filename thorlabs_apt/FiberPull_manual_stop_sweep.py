...
def pullFiber():
    ...
    torch_on_fiber = float(input("Enter the desired torch position (10 default): ")) #time t
    pullTime = float(input("Enter the desired pull time in seconds: ")) #time to pull
    ...
    sweepSpeed = 2
    sweepTime = 1.2
    sweepLength = 6 # this isn't really how far it will sweep, it just needs to be a bit bigger than the desired length
    tic = time.time()
    toc = 0
    motorLeft.set_velocity_parameters(0.001,4.0,sweepSpeed)
    motorRight.set_velocity_parameters(0.001,4.0,sweepSpeed)
    while toc < warmUpTime:
        motorLeft.move_by(-sweepLength)
        motorRight.move_by(sweepLength)
    ...
    while toc < pullTime:
        motorLeft.set_velocity_parameters(0.001,4.0,sweepSpeed)
        motorRight.set_velocity_parameters(0.001,4.0,sweepSpeed+PullSpeed)
        motorLeft.move_by(-sweepLength)
        motorRight.move_by(sweepLength)
        time.sleep(sweepTime)
        motorLeft.set_velocity_parameters(0.001,4.0,sweepSpeed+PullSpeed)
        motorRight.set_velocity_parameters(0.001,4.0,sweepSpeed)
        motorLeft.move_by(sweepLength)
        motorRight.move_by(-sweepLength)
        time.sleep(sweepTime)
...
while True:
    ...
    if command is 't':
        ...
        if (leftPosition+tenTensionLength > 25) or (rightPosition+(tenTensionLength) > 25):
            print("Can't tension 10.")
            continue
        motorLeft.move_by(tenTensionLength)
        motorRight.move_by(tenTensionLength)
        while motorLeft.is_in_motion:
            pass
        while motorRight.is_in_motion:
            pass
